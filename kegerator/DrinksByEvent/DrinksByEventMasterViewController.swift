//
// DrinksByEventMasterViewController.swift
// kegerator
//
...
class DrinksByEventMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var barcelona: Barcelona<OnlineODataProvider> {
        return self.appDelegate.barcelona
    }

    private var entities: [DrinksByEventType] = [DrinksByEventType]()
    private let logger = Logger.shared(named: "DrinksByEventTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let drinksbyeventtype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "TOTAL_DRINKS", value: "\(drinksbyeventtype.totalDrinks!)")
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        self.barcelona.deleteEntity(currentEntity) { error in
            if let error = error {
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.barcelona.fetchDrinksByEvent(matching: query) { drinksByEvent, error in
            guard let drinksByEvent = drinksByEvent else {
                completionHandler(error!)
                return
            }
            self.entities = drinksByEvent
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! DrinksByEventDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].totalDrinks != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].totalDrinks!)" : "")"
            ...
            let detailViewController = dest.viewControllers[0] as! DrinksByEventDetailViewController
