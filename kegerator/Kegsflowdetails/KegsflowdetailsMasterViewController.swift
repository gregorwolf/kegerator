//
// KegsflowdetailsMasterViewController.swift
// kegerator
//
...
class KegsflowdetailsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var barcelona: Barcelona<OnlineODataProvider> {
        return self.appDelegate.barcelona
    }

    private var entities: [KegsflowdetailsType] = [KegsflowdetailsType]()
    private let logger = Logger.shared(named: "KegsflowdetailsTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let kegsflowdetailstype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ID", value: "\(kegsflowdetailstype.id!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.barcelona.fetchKegsflowdetails(matching: query) { kegsflowdetails, error in
            guard let kegsflowdetails = kegsflowdetails else {
                completionHandler(error!)
                return
            }
            self.entities = kegsflowdetails
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! KegsflowdetailsDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! KegsflowdetailsDetailViewController
