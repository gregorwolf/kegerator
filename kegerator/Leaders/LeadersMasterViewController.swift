//
// LeadersMasterViewController.swift
// kegerator
//
...
class LeadersMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var barcelona: Barcelona<OnlineODataProvider> {
        return self.appDelegate.barcelona
    }

    private var entities: [LeadersType] = [LeadersType]()
    private let logger = Logger.shared(named: "LeadersTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let leaderstype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "DRINKS", value: "\(leaderstype.drinks!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.barcelona.fetchLeaders(matching: query) { leaders, error in
            guard let leaders = leaders else {
                completionHandler(error!)
                return
            }
            self.entities = leaders
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! LeadersDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].drinks != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].drinks!)" : "")"
            ...
            let detailViewController = dest.viewControllers[0] as! LeadersDetailViewController
