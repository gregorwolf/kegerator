//
// LeaderboardMasterViewController.swift
// kegerator
//
...
class LeaderboardMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var barcelona: Barcelona<OnlineODataProvider> {
        return self.appDelegate.barcelona
    }

    private var entities: [LeaderboardType] = [LeaderboardType]()
    private let logger = Logger.shared(named: "LeaderboardTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let leaderboardtype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ID", value: "\(leaderboardtype.id!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.barcelona.fetchLeaderboard(matching: query) { leaderboard, error in
            guard let leaderboard = leaderboard else {
                completionHandler(error!)
                return
            }
            self.entities = leaderboard
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! LeaderboardDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! LeaderboardDetailViewController
