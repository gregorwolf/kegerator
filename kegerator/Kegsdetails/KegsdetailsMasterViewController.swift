//
// KegsdetailsMasterViewController.swift
// kegerator
//
...
class KegsdetailsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var barcelona: Barcelona<OnlineODataProvider> {
        return self.appDelegate.barcelona
    }

    private var entities: [KegsdetailsType] = [KegsdetailsType]()
    private let logger = Logger.shared(named: "KegsdetailsTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let kegsdetailstype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ID", value: "\(kegsdetailstype.id!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.barcelona.fetchKegsdetails(matching: query) { kegsdetails, error in
            guard let kegsdetails = kegsdetails else {
                completionHandler(error!)
                return
            }
            self.entities = kegsdetails
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! KegsdetailsDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! KegsdetailsDetailViewController
