//
// FlowsumMasterViewController.swift
// kegerator
//
...
class FlowsumMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var barcelona: Barcelona<OnlineODataProvider> {
        return self.appDelegate.barcelona
    }

    private var entities: [FlowsumType] = [FlowsumType]()
    private let logger = Logger.shared(named: "FlowsumTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let flowsumtype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "FLOWSUM", value: "\(flowsumtype.flowsum!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.barcelona.fetchFlowsum(matching: query) { flowsum, error in
            guard let flowsum = flowsum else {
                completionHandler(error!)
                return
            }
            self.entities = flowsum
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! FlowsumDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].flowsum != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].flowsum!)" : "")"
            ...
            let detailViewController = dest.viewControllers[0] as! FlowsumDetailViewController
