//
// DrinksByKegMasterViewController.swift
// kegerator
//
...
class DrinksByKegMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator, TableUpdaterDelegate {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var barcelona: Barcelona<OnlineODataProvider> {
        return self.appDelegate.barcelona
    }

    private var entities: [DrinksByKegType] = [DrinksByKegType]()
    private let logger = Logger.shared(named: "DrinksByKegTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let drinksbykegtype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "TOTAL_DRINKS", value: "\(drinksbykegtype.totalDrinks!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.barcelona.fetchDrinksByKeg(matching: query) { drinksByKeg, error in
            guard let drinksByKeg = drinksByKeg else {
                completionHandler(error!)
                return
            }
            self.entities = drinksByKeg
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! DrinksByKegDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! DrinksByKegDetailViewController
